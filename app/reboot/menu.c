...
static void draw_menu(void) {
	...
	gfx_draw_rectangle(margin_x, margin_y, frame_width, frame_height, 0xFFFFFF);
	...
	for (i = 0; i < num_of_boot_entries; i++) {
		...
		gfx_draw_filled_rectangle(margin_x + 8, (margin_y + 8) + i * (ACTUAL_FONT_HEIGHT + 4) - 1, frame_width - (2 * 8), 2 + ACTUAL_FONT_HEIGHT + 2, highlight_color);
		gfx_draw_text(margin_x + 10, (margin_y + 10) + i * (ACTUAL_FONT_HEIGHT + 4), (entry_list + i)->title, BOOT_ENTRY_SCALE, font_color);
	}

	uint32_t separator_y = (margin_y + 8) + i * (ACTUAL_FONT_HEIGHT + 4) + 2;

	gfx_draw_horizontal_line(margin_x + 8, (margin_x + 8) + (frame_width - (2 * 8)), separator_y, 0xFFFFFF);
	...
	for (i = 0; i < HARDCODED_ENTRY_COUNT; i++) {
		...
		gfx_draw_filled_rectangle(margin_x + 8, (separator_y + 3) + i * (ACTUAL_FONT_HEIGHT + 4), frame_width - (2 * 8), 2 + ACTUAL_FONT_HEIGHT + 2, highlight_color);
		gfx_draw_text(margin_x + 10, (separator_y + 5) + i * (ACTUAL_FONT_HEIGHT + 4), hardcoded_entry_list[i].title, BOOT_ENTRY_SCALE, font_color);
	}
